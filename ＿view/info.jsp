	...
	<div class="container">
			<div class="row">
				<div class="col-xs-12">
					...
					<input type="hidden" value="${info.mno }" id="mno"> <input
						type="hidden" value="${sessionScope.loginId }" id="id">
					<h2>${info.title }</h2>
					<table id="scoreTable">
					</table>
					<img src="${info.poster }">
					<h3>감독: ${info.director } </h3> 
					<h3>장르: ${info.genre } </h3>
					<h3>배우: ${info.actor } </h3>  
					...
					<c:if test="${sessionScope.loginId != null }">
						...
						<input type="button" value="한줄평 등록" onclick="upload()">
					</c:if>
				</div>
				</div>
			</div>
